...
export default function LayerBrowser({ imageName, digest, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    getLayerEntries(imageName, digest)
      ...
      .catch(err => { if (!cancelled) setError(err instanceof Error ? err.message : 'Failed to load layer contents') })
  ...
  return (
    <Dialog open onOpenChange={open => { if (!open) onClose() }}>
      <DialogContent className="sm:max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="text-sm">Layer contents</DialogTitle>
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground py-10 text-center">
            This layer has no files (metadata-only change).
          </p>
        ) : (
          <>
            <div className="relative shrink-0">
              ...
              <Input
                autoFocus
                placeholder={`Filter ${entries?.length ?? 0} entries…`}
                value={filter}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto -mx-4 px-4 border-t">
              {filtered.length === 0 ? (
                <p className="text-xs text-muted-foreground py-6 text-center">No entries match "{filter}"</p>
              ) : (
